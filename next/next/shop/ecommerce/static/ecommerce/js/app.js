...
var app = {
    ...
    Posts: function () {
        ...
        this.init = function () {
            ...
            this.get_all_category_pagination();
            this.get_all_subcategory_pagination();
        ...
        this.get_all_subcategory_pagination = function () {
            ...
            if ($('form.post-list input').val()) {
                ...
                _this.ajax_get_all_subcategory_pagination(data.page, data.name, data.sort);
            } else {
                ...
                /* Load first page */
                _this.ajax_get_all_subcategory_pagination(page, $('.post_name').val(), $('.post_sort').val());
            }

            /* Search */
            $('body').on('click', '.product_search_submit', function () {
                _this.ajax_get_all_subcategory_pagination(1, $('.post_name').val(), $('.post_sort').val());
            });
         ...
         */
        this.ajax_get_all_subcategory_pagination = function (page, order_by_name, order_by_sort) {

            if ($(".pagination-container").length > 0 && $('.subcategory-view-all').length > 0) {
                $(".pagination-container").html(item_pagination_preloader());

                var post_data = {
                    page: page,
                    search: $('.product_search_text').val(),
                    name: order_by_name,
                    sort: order_by_sort,
                    max: $('.post_max').val(),
                };
                ...
                $.ajax({
                    url: module_path + 'subcategory/' + subcategory_slug,
                    type: 'POST',
                    data: data,
                    success: function (response) {
                        if ($(".pagination-container").html(response.subcategory_content)) {
                            $('.pagination-nav').html(response.subcategory_navigation);
                            scroll_to("body")
        ...
        this.get_all_category_pagination = function () {
            ...
            if ($('form.post-list input').val()) {
                /* Submit hidden form input value to load previous page number */
                data = JSON.parse($('form.post-list input').val());
                _this.ajax_get_all_category_pagination(data.page, data.name, data.sort);
            } else {
                /* If there's a page set in the URL, we load it. */
                page = 1;
                page_from_url = get_url_value('page');
                ...
                /* Load first page */
                _this.ajax_get_all_category_pagination(page, $('.post_name').val(), $('.post_sort').val());
            }

            /* Search */
            $('body').on('click', 'product_search_submit', function () {
                _this.ajax_get_all_category_pagination(1, $('.post_name').val(), $('.post_sort').val());
            });
            /* Search when Enter Key is triggered */
            $(".product_search_text").keyup(function (e) {
                if (e.keyCode == 13) {
         ...
         * AJAX front-end items pagination.
         */
        this.ajax_get_all_category_pagination = function (page, order_by_name, order_by_sort) {

            if ($(".pagination-container").length > 0 && $('.category-view-all').length > 0) {
                $(".pagination-container").html(item_pagination_preloader());
                ...
                $.ajax({
                    url: module_path + 'get_category/' + category_slug,
                    type: 'POST',
                    data: data,
                    success: function (response) {
                        if ($(".pagination-container").html(response.category_content)) {
                            $('.pagination-nav').html(response.category_navigation);
                            scroll_to("body")
                            $('.table-post-list th').each(function () {
                                /* Append the button indicator */
                                $(this).find('i.fas').remove();
        ...
        this.add_post = function () {
            ...
            if (get_url_value('status') == 'created') {
                Lobibox.notify('success', {
                    msg: 'Item successfully created, you may continue editing this product.',
                    size: 'mini',
                    sound: false
                });
            }
        ...
        this.set_featured_image = function () {
            $('body').on('click', '.set-featured-image', function (e) {
                ...
                if ($(this).hasClass('fas')) {
                    Lobibox.notify('error', {
                        msg: 'The image you clicked is already the featured image.',
                        size: 'mini',
                        sound: false
                    });
                    wave_box('off');
